...
template <bool UseLowerBounds>
void a_star<UseLowerBounds>::add_start(location_idx_t l, 
                                       unixtime_t t) {
  // Used from query_engine
  ...
  for (auto const r : tt_.location_routes_[l]) {
    ...
    for (auto i = stop_idx_t{0U}; i < stop_seq.size() - 1; ++i) {
      ...
      state_.pq_.push(
          queue_entry{transport_first_segment + et.stop_seq_idx_, 0U});
    }
  }
}

// template <bool UseLowerBounds>
// void a_star<UseLowerBounds>::reconstruct(query const& q, 
//                                          journey& j) const {
