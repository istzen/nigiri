...
a_star::a_star(
    ...
    : tt_{tt},
      ...
      lb_{lb},
      base_{base} { // TODO: in query engine this is QUERY_DAY_SHIFT check why needed and what is does
...
void a_star::add_start(location_idx_t l, 
                       unixtime_t t) {
  // Used from query_engine
  ...
  for (auto const r : tt_.location_routes_[l]) {
    ...
    for (auto i = stop_idx_t{0U}; i < stop_seq.size() - 1; ++i) {
      ...
      state_.arrival_day_.emplace(transport_first_segment, day_idx_t{delta.days()});
      state_.arrival_time_.emplace(transport_first_segment, minutes_after_midnight_t{delta.mam()});
      // Update Predecessor Table
      state_.pred_table_.emplace(transport_first_segment, state_.sartSegmentPredecessor);
