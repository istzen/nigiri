...
struct cost_function {
  ...
  cost_function(minutes_after_midnight_t const st,
                day_idx_t const sd,
                float const tf = 1.0F,
                arrival_time_map const* at = nullptr,
                day_index_map const* ad = nullptr)
      : arrival_time_{at},
        arrival_day_{ad},
        transfer_factor_{tf},
        start_day_{sd},
  ...
  dist_t operator()(queue_entry const& q) const {
    ...
    auto const time_travelled = static_cast<dist_t>(
        (arrival_day_->find(q.segment_)->second.v_ - start_day_.v_) * 24 * 60 +
        arrival_time_->find(q.segment_)->second.count() - start_time_.count());
    return (time_travelled + transfer_factor_ * q.transfers_);
  }

  arrival_time_map const* const arrival_time_;  // segment -> arrival_time
  day_index_map const* const arrival_day_;  // segment -> arrival_day
  ...
  minutes_after_midnight_t const
      start_time_;  // minutes_after_midnight_t of start_time
...
struct a_star_state {
  ...
  a_star_state(tb_data const& tbd)
      : tbd_{tbd},
        pq_{maxASTravelTime.count(),
            cost_function{minutes_after_midnight_t{0}, day_idx_t{0}, 1.0F,
                          &arrival_time_, &arrival_day_}} {
    end_reachable_.resize(tbd.segment_transfers_.size());
  }

  // TODO: check if needed
  void update_segment_arr_time(segment_idx_t const s, duration_t const d) {
  ...
  ;  // segment -> distance to destination
